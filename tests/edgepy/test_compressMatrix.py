...
class test_DGEList(unittest.TestCase):
    def test_makeCompressedMatrix(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex="dims\[.\] should be equal to length of x"):
            makeCompressedMatrix([1,2,3], dims=(3,2), byrow=True)
        with self.assertRaisesRegex(ValueError, expected_regex="dims\[.\] should be equal to length of x"):
            makeCompressedMatrix([1,2,3], dims=(2,3), byrow=False)
        with self.assertRaisesRegex(ValueError, expected_regex="dims does not represent the shape of a matrix"):
            makeCompressedMatrix(42, dims=(5,))
        with self.assertRaisesRegex(ValueError, expected_regex="dims does not represent the shape of a matrix"):
            makeCompressedMatrix(42, dims=(1,2,3))
        with self.assertRaisesRegex(ValueError, expected_regex="input has too many dimensions to be interpreted as a matrix"):
