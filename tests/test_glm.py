...
class test_nbinomDeviance(unittest.TestCase):
    ...
    def test_nbinomDeviance(self):
        dev_ref = np.array([7.999999,5.545177,321.818901,184.761910,355.515748,340.390858,414.784086,461.324010,235.746631,354.718673,410.643778,265.824737,422.344282,412.896876,392.316278,599.686176,242.088733,501.330767,608.676351,706.581453,430.474599,310.504051])
        ...
        with self.assertRaisesRegex(ValueError, expected_regex="y is a matrix but mean is not"):
            nbinomDeviance(self.y, np.ones((22,)))

        with self.assertRaisesRegex(ValueError, expected_regex="mean is a matrix but y is not"):
            nbinomDeviance(np.array([1,2,3,4]), np.ones((4,3)))

        with self.assertRaisesRegex(ValueError, expected_regex="length of mean differs from that of y"):
...
class test_addPriorCount(unittest.TestCase):
    ...
    def test_addPriorCount(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex="count matrix must be numeric"):
...
class test_glmFit(unittest.TestCase):
    ...
    def test_glmFit(self):
        with self.assertRaisesRegex(ValueError, expected_regex="No dispersion values found in DGEList object"):
            self.d.glmFit()
        ...
        self.assertTrue(np.allclose(e.coefficients, coef_ref, atol=1e-5, rtol=0, equal_nan=True))

        with self.assertRaisesRegex(ValueError, expected_regex="design should have as many rows as y has columns"):
            glmFit(self.d.counts, design=np.ones((5,1)))

        with self.assertRaisesRegex(ValueError, expected_regex="No dispersion values provided"):
            glmFit(self.d.counts, design=design)

        with self.assertRaisesRegex(ValueError, expected_regex="Dimensions of dispersion do not agree with dimensions of y"):
            glmFit(self.d.counts, design=design, dispersion=np.ones((5,1)))

        with self.assertRaisesRegex(ValueError, expected_regex="Dimensions of offset do not agree with dimensions of y"):
            glmFit(self.d.counts, design=design, dispersion=0.05, offset=np.ones((5,1)))

        with self.assertRaisesRegex(ValueError, expected_regex="lib_size has wrong length, should agree with ncol\(y\)"):
            glmFit(self.d.counts, design=design, dispersion=0.05, lib_size=np.ones((2,)))


        e = glmFit(self.d.counts, dispersion=0.05, prior_count=0)
        coef_ref = np.array([-1.000000e+08,-6.099236e+00,-3.120865e+00,-3.495951e+00,-3.138641e+00,-3.114458e+00,-2.953059e+00,-2.908932e+00,-3.325914e+00,-3.096400e+00,-2.953421e+00,-3.252006e+00,-2.951040e+00,-2.960371e+00,-2.980034e+00,-2.691442e+00,-3.309082e+00,-2.895371e+00,-2.686176e+00,-2.563754e+00,-2.917718e+00,-3.142277e+00]).reshape(e.coefficients.shape)
        self.assertTrue(np.allclose(e.coefficients, coef_ref, atol=1e-6, rtol=0))
