...
class Test(unittest.TestCase):
    ...
    def test_lmFit_with_missing_values(self):
        """Test lmFit handles missing values (NaN) in expression data correctly.
        
        This test ensures that .iloc indexing is used correctly when dealing with
        missing values, avoiding pandas issues
        """
        # Create expression data with missing values
        y_with_nan = self.y.copy()
        y_with_nan.iloc[0, 0] = np.nan  # gene0, sample0
        y_with_nan.iloc[0, 1] = np.nan  # gene0, sample1
        y_with_nan.iloc[2, 3] = np.nan  # gene2, sample3
        y_with_nan.iloc[5, 2] = np.nan  # gene5, sample2
        
        fit = lmFit(y_with_nan, self.design)
        
        # Reference coefficients 
        coef_ref = pd.DataFrame(
            {
                "group[1]": [
                    0.19430656143020775, 0.18771731587253482, -0.3396235805604797, -0.08546790079349487, -0.15844537122376193, 0.1875857358419311, -0.307899257777345, -0.1895273968344957, -0.10953378963994775, 0.1123062118389511
                ],
                "group[2]": [
                    0.10547395247355086, -0.03865874027967545, -0.10478756816087245, -0.1582996282012069, -0.05166344213664099, -0.14560096668237335, 0.11066960970545048, -0.012336075313558055, -0.04503279918814075, 0.09975961826747089
                ],
            },
            index=self.y.index,
        )
        ...
        # Reference df_residual 
        df_residual_ref = [
           2.0, 4.0, 3.0, 4.0, 4.0, 3.0, 4.0, 4.0, 4.0, 4.0
        ]
        self.assertTrue(np.array_equal(fit.df_residual, df_residual_ref))
        
        # Reference sigma 
        sigma_ref = [
                0.3043516416417704, 0.2512174148516086, 0.33452763527420765, 0.36662325086737063, 0.17067351092144448, 0.4095176821829878, 0.24616182108649962, 0.25699992083891215, 0.29411301557005015, 0.2615085227616588
            ],
        self.assertTrue(np.allclose(fit.sigma, sigma_ref, rtol=1e-6))
        
        # Reference cov_coefficients 
        cov_ref = pd.DataFrame(
            {
                0: [
        ...
        pd.testing.assert_frame_equal(pd.DataFrame(fit.cov_coefficients), cov_ref, rtol=1e-6)
        
        # Reference stdev_unscaled 
        stdev_ref = pd.DataFrame(
            {
                "group[1]": [
                   1.0, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.7071067811865476, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257
                ],
                "group[2]": [
                   0.5773502691896257, 0.5773502691896257, 0.7071067811865476, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257, 0.5773502691896257
                ],
            },
            index=self.y.index,
        )
        pd.testing.assert_frame_equal(fit.stdev_unscaled, stdev_ref, rtol=1e-6)
        
        # Reference Amean 
        Amean_ref = pd.Series(
            [
                0.1276821047127151, 0.07452928779642966, -0.24568917560063683, -0.12188376449735087, -0.10505440668020145, -0.012326285672651538, -0.09861482403594729, -0.10093173607402688, -0.07728329441404425, 0.10603291505321098
            ],
            index=self.y.index,
        )
        pd.testing.assert_series_equal(fit.Amean, Amean_ref, rtol=1e-6)
        
        # Ensure indices are preserved correctly
        pd.testing.assert_index_equal(fit.coefficients.index, y_with_nan.index)
        pd.testing.assert_index_equal(fit.stdev_unscaled.index, y_with_nan.index)
        pd.testing.assert_index_equal(
            fit.stdev_unscaled.columns, fit.coefficients.columns
        )
