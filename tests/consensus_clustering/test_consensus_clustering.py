...
class test_consensusClustering(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.CC = consensusClustering(cluster=AgglomerativeClustering, mink=2, maxk=4, nb_resampling_iteration=50, resample_proportion=0.5)
    ...
    def test_internal_resample(self):
        resampled_indices = self.CC._internal_resample(self.mocked_data, 0.5, np.random.default_rng())

        assert len(resampled_indices) == len(self.mocked_data)/2

    def test_compute_consensus_clustering(self):
        self.CC.compute_consensus_clustering(self.mocked_data.to_numpy(), random_state=0)

        # test consensus matrix is symetric
        for i in range(len(self.CC.consensus_matrices)):
            assert np.allclose(self.CC.consensus_matrices[i], self.CC.consensus_matrices[i].T)
