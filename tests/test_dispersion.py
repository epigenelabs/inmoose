...
class test_dispersion(unittest.TestCase):
    ...
    def test_maximizeInterpolant(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex="y is not a matrix: cannot perform interpolation"):
            maximizeInterpolant(spline_pts, np.array([1,2,3]))
        with self.assertRaisesRegex(ValueError, expected_regex="number of columns must equal number of spline points"):
            maximizeInterpolant(spline_pts, np.ones((2,3)))
        with self.assertRaisesRegex(ValueError, expected_regex="spline points must be unique and sorted"):
            maximizeInterpolant([3,2,1], np.ones((2,3)))
        with self.assertRaisesRegex(ValueError, expected_regex="spline points must be unique and sorted"):
            maximizeInterpolant([1,2,2], np.ones((2,3)))

        ref = [0.0000000,5.2951564,0.0000000,1.2921216,1.6173398,5.3493726,5.1489371,1.6854523,5.0732169,7.0000000,0.0000000,1.3839820,0.0000000,0.7054761,0.0000000,5.2193349,1.0046347,1.6395426,5.0557785,4.8235934,7.0000000,4.3981600]
    ...
    def test_dispCoxReid(self):
        ...
        self.assertAlmostEqual(dispCoxReid(self.d.counts, tol=1e-15), 0.158033594104552, 7)
        self.assertAlmostEqual(dispCoxReid(self.d.counts, subset=5, tol=1e-15), 0.091028284006027, 7)

        with self.assertRaisesRegex(ValueError, expected_regex="no data rows with required number of counts"):
            dispCoxReid(self.d.counts, min_row_sum=1000)
        with self.assertRaisesRegex(ValueError, expected_regex="please give a non-negative interval for the dispersion"):
    ...
    def test_movingAverageByCol(self):
        ...
        with self.assertWarnsRegex(Warning, expected_regex="reducing moving average width to x.shape\[0\]"):
